...
import kingmc.util.errorprone.IncompatibleModifiers
import kingmc.util.errorprone.Modifier
import java.lang.annotation.Retention
import java.lang.annotation.RetentionPolicy

/**
 * Use this annotation on any static or field that will be initialized lazily, where races yield no
 * semantic difference in the code (as, for example, is the case with [String.hashCode]). Note
 ...
 * then, always using the pattern as presented in the `getData` method of this sample code
 * below:
 *
 * <pre>`private final String source;
 ...
 *
 *
 * The need for using the `local` variable is detailed in
 ...
 *
 *
 * Also note that `LazyInit` must not be used on 64-bit primitives (`long`s and
 * `double`s), because the Java Language Specification does not guarantee that writing to
