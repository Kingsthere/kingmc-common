...
fun Collection<Annotation>.hasAnnotationClass(targetAnnotation: KClass<out Annotation>): Boolean {
    return any { annotation ->
        REFLECTION_ANNOTATION_ACCESSOR.hasAnnotation(
 ...
 * Get an annotation from this class by the type of the annotation
 */
inline fun <reified T : Annotation> KAnnotatedElement.getAnnotationWithFormattedProperty(
 ...
 */
@Suppress("UNCHECKED_CAST")
inline fun <reified T : Annotation> KAnnotatedElement.getAnnotationsWithFormattedProperty(
    formatContext: FormatContext,
    formatter: Formatter = BracketStyle
): List<T> =
 ...
 * Get an annotation from this list of annotations by the type of the annotation
 */
inline fun <reified T : Annotation> List<Annotation>.getAnnotationWithFormattedProperty(
    formatContext: FormatContext,
    formatter: Formatter = BracketStyle
) =
 ...
 */
@Suppress("UNCHECKED_CAST")
inline fun <reified T : Annotation> List<Annotation>.getAnnotationsWithFormattedProperty(
    formatContext: FormatContext,
    formatter: Formatter = BracketStyle
): List<T> =
...
fun List<Annotation>.getAnnotationsByClass(targetAnnotation: KClass<out Annotation>): List<Annotation> {
    val annotations = filter { annotation ->
        REFLECTION_ANNOTATION_ACCESSOR.hasAnnotation(
            annotation.annotationClass,
            targetAnnotation
        )
 ...
 * Instantiate an enhanced `Annotation` from this list of annotations that is or inherited [targetAnnotation]
 */
fun Collection<Annotation>.getAnnotationByClass(
 ...
 * Instantiate a list of `Annotation`s from this list of annotations that is or inherited [targetAnnotation]
 */
fun List<Annotation>.getAnnotationsByClass(
    targetAnnotation: KClass<out Annotation>,
    formatContext: FormatContext,
    formatter: Formatter = BracketStyle
): List<Annotation> {
    ...
    return annotations.map { annotationFound ->
        ANNOTATION_ENHANCER(
            annotationFound,
            targetAnnotation,
            formatContext,
