...
public class RuntimeEnv {
    ...
    // Enable this to scan the sub dependencies that is dependent that
    // dependencies defined pom.xml file
    ...
    public void loadAssets(String name, String hash, String url, boolean zip) {
        ...
        System.out.println("Downloading assets " + url.substring(url.lastIndexOf('/') + 1));
        try {
            if (zip) {
    ...
    public void loadDependency(@NotNull String url, @NotNull File baseDir, @NotNull List<JarRelocation> relocation, @Nullable String repository, boolean ignoreOptional, boolean ignoreException, boolean transitive, @NotNull DependencyScope[] dependencyScopes) throws IOException {
        ...
        if (enableDeepDependencyScan) {
            ...
            if (pomFile.exists() && pomShaFile.exists() && DependencyDownloader.readFile(pomShaFile).startsWith(DependencyDownloader.readFileHash(pomFile))) {
                ...
                try {
                    System.out.printf("Downloading library %s:%s:%s %s%n", args[0], args[1], args[2], transitive ? "(transitive)" : "");
        ...
        if (transitive) {
            downloader.injectClasspath(downloader.loadDependency(downloader.getRepositories(), current), false);
        } else {
            downloader.injectClasspath(Collections.singleton(current), false);
        }
    }
}
